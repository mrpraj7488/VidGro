...
export default function AnalyticsTab() {
  ...
  const fetchAnalytics = async (isRefresh = false) => {
    ...
    try {
      ...
      // Fetch promoted videos with GUARANTEED real-time analytics
      ...
      // Get GUARANTEED fresh analytics data for each video
      const videosWithAnalytics = await Promise.all(
        (promotedVideos || []).map(async (video) => {
          try {
            // Get GUARANTEED real-time analytics for this video
            ...
            if (analyticsError) {
              console.error('Error fetching video analytics:', analyticsError);
              // Enhanced fallback with fresh view count
              ...
              const freshVideoData = freshVideo || video;
              return {
                ...freshVideoData,
                display_views_count: freshVideoData.views_count,
                progress_text: `${freshVideoData.views_count}/${freshVideoData.target_views}`,
                fresh_data: false
              };
            }
            ...
            if (analyticsData && !analyticsData.error) {
              return {
                ...video,
                ...analyticsData,
                display_views_count: analyticsData.views_count || video.views_count,
                progress_text: analyticsData.progress_text || `${video.views_count}/${video.target_views}`,
                fresh_data: analyticsData.fresh_data || true
              };
            }

            // Final fallback with current data
            return {
              ...video,
      ...
      // Fetch filtered coin transactions (excluding video_watch for cleaner activity feed)
      const { data: transactions, error: transactionsError } = await supabase
        .rpc('get_filtered_recent_activities', {
          user_uuid: user.id,
          activity_limit: 20
        });
      ...
      // Get total coins earned from all sources for display
      ...
      const totalCoinsEarned = allTransactions
        ?.filter(t => t.amount > 0)
      ...
      setAnalytics({
        totalVideosPromoted,
        totalCoinsEarned,
        recentActivities: formattedTransactions,
      ...
      const freshDataCount = videosWithAnalytics.filter(v => v.fresh_data).length;
      console.log(`📊 Analytics refreshed: ${freshDataCount}/${videosWithAnalytics.length} videos with fresh data`);
    } catch (error) {
    ...
    setHoldTimers(prev => {
      ...
      Object.keys(updated).forEach(videoId => {
        if (updated[videoId] > 0) {
          updated[videoId] = Math.max(0, updated[videoId] - 30); // Decrease by 30 seconds
          hasChanges = true;
