/*
  # Fix Video Timing Precision Issue - CORRECTED VERSION
  ...
  4. Fixed RAISE NOTICE parameter issues
*/
...
BEGIN
  ...
  RAISE NOTICE 'Starting coin award for user % watching video % for % seconds', user_uuid, video_uuid, watch_duration;
  ...
  IF watch_duration < completion_threshold THEN
    RAISE NOTICE 'User % did not watch enough of video % (watched: % seconds, required: % seconds, target: % seconds)', 
  ...
  RAISE NOTICE 'Calculated % coins for % second video (watched % seconds, threshold % seconds)', 
  ...
  RAISE NOTICE 'Successfully awarded % coins to user % for video % (watched %/%)', 
...
BEGIN
  ...
  RAISE NOTICE 'Users watching 44+ seconds of 45-second videos will now earn coins';
