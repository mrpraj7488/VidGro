/*
  # Complete VidGro Database Setup - Fresh Installation
  
  This script sets up the entire VidGro database from scratch including:
  
  1. Tables
    - profiles (user profiles and coin balances)
    - videos (promoted videos with hold system)
    - video_views (tracking video watches)
...
CREATE INDEX IF NOT EXISTS idx_videos_active_views ON videos(status, views_count, target_views) WHERE status = 'active';
...
CREATE INDEX IF NOT EXISTS idx_video_views_realtime_analytics ON video_views(video_id, completed, created_at);
...
-- Enhanced function to safely update user coins (FIXED)
...
CREATE OR REPLACE FUNCTION enhanced_complete_video_view_v2(
  user_uuid uuid,
  video_uuid uuid,
...
BEGIN
  ...
  calculated_coins := CASE 
    WHEN is_completed THEN calculate_coins_by_duration_v2(video_record.duration_seconds)
    ELSE 0 
  END;
  ...
  IF is_completed THEN
    ...
    PERFORM update_video_engagement(video_uuid);
  END IF;
...
BEGIN
  ...
  SELECT 
    ...
    v.title,
    v.duration_seconds,
    calculate_coins_by_duration_v2(v.duration_seconds) as coin_reward -- Use calculated coins
  FROM videos v
...
CREATE OR REPLACE FUNCTION force_refresh_user_profile(user_uuid uuid)
RETURNS json AS $$
DECLARE
  user_data RECORD;
BEGIN
...
BEGIN
  ...
  PERFORM update_video_engagement(affected_video_id);
...
BEGIN
  ...
  RAISE NOTICE '📊 Created tables: profiles, videos, video_views, coin_transactions, user_settings';
  RAISE NOTICE '⚙️  Created functions: All essential functions for video promotion, coin management, analytics';
  RAISE NOTICE '🔒 Enabled Row Level Security with appropriate policies';
  RAISE NOTICE '🚀 Added performance indexes and triggers';
  RAISE NOTICE '👑 VIP system enabled with subscription management';
  RAISE NOTICE '⏱️  10-minute hold system for new video promotions';
  RAISE NOTICE '💰 Enhanced coin calculation based on duration mapping';
  RAISE NOTICE '📈 Real-time analytics and engagement tracking';
  RAISE NOTICE '✅ Your VidGro database is ready for production!';
END $$;