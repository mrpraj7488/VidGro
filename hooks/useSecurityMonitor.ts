...
export function useSecurityMonitor() {
  ...
  const handleSecurityViolation = (reason: string) => {
    ...
    Alert.alert(
      'Security Warning',
      `${reason}. Some features may be restricted for security reasons.`,
      [
        { text: 'OK' },
        { 
          text: 'Learn More', 
          onPress: () => {
            // Navigate to security info screen
          }
        }
      ]
    );
  ...
  const handleAdBlockDetection = () => {
    ...
    Alert.alert(
      'Ad Blocker Detected',
      'Ad blocking software has been detected. This may affect your ability to earn free coins.',
      [
        { text: 'OK' },
        { 
          text: 'Disable Ad Blocker', 
          onPress: () => {
            // Provide instructions for disabling ad blocker
          }
        }
      ]
    );
