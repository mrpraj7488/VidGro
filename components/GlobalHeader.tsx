...
export default function GlobalHeader({ title, showCoinDisplay = true, menuVisible, setMenuVisible }: GlobalHeaderProps) {
  const { user, profile, signOutAndRedirect, refreshProfile } = useAuth();
  ...
  const handleMenuPress = () => {
    console.log('🔍 Menu button pressed, current menuVisible:', menuVisible);
    if (menuVisible) {
      console.log('⚠️ Menu already visible, ignoring press');
      return;
    }
    setMenuVisible(true);
  ...
  const handleCloseMenu = () => {
    console.log('🔍 Closing menu, current menuVisible:', menuVisible);
    if (!menuVisible) {
      console.log('⚠️ Menu already closed, ignoring close');
      return;
    }
